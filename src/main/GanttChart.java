...
// taken from https://stackoverflow.com/questions/27975898/gantt-chart-from-scratch
public class GanttChart<X,Y> extends XYChart<X,Y> {
    ...
    private Node createContainer(Series<X, Y> series, int seriesIndex, final Data<X,Y> item, int itemIndex) {
        ...
        if (container == null) {
            ...
            if(extraValue.job!=-1) {
                Text text = new Text(extraValue.job + "\n" + extraValue.activity);
                text.setFont(Font.font(null, FontWeight.BOLD, 10));
                s.getChildren().add(text);
                StackPane.setMargin(text, new Insets(40, 0, 0, 15));
            }
